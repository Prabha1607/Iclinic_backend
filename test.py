...
def _build_appointment_types(appointment_types: list) -> dict:
    return {at.id: (at.name, at.description or "") for at in appointment_types if at.is_active}
...
async def chat_loop():
    ...
    print(f"[debug] loaded {len(built_appointment_types)} appointment types: {list(built_appointment_types.keys())}")
