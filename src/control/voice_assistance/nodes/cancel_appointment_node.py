...
async def _handle_ask_confirm(state: dict, user_text: str) -> dict:
    ...
    try:
        ...
        print(f"[cancel_appointment_node] Raw decision: '{raw_decision}' → Parsed: '{decision}'")
...
async def cancel_appointment_node(state: dict) -> dict:
    ...
    if state.get("cancellation_awaiting_fresh_input"):
        print("[cancel_appointment_node] Awaiting fresh user input — skipping confirmation check.")
    ...
    print(f"[cancel_appointment_node] WARNING: Unhandled stage='{stage}' — passing through.")
    return state